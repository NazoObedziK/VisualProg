		...
		private void button_build_Click(object sender, EventArgs e)
		{
          ...
          //  Масиви Y – по одному для кожного графіка
			...
			{// вичисляємо Х на кожному кроці
                ...
                y2[i] = Math.Pow(x[i],1.0/3.0);//x(t) для параметрично заданої функції
                y3[i] = Math.Pow((1-x[i]), 1.0 / 3.0);//y(t)  для параметрично заданої функції
				y4[i] = Math.Pow((1 - Math.Pow(x[i], 1.0 / 3.0)), 1.0 / 3.0);//x(y)  для параметрично заданої функції
				y5[i] = Math.Pow((1 - x[i]), 1.0 / 9.0);//y(x)  для параметрично заданої функції
				...
				dataGridView4.Rows.Add(x[i], y4[i]);
				dataGridView5.Rows.Add(x[i], y5[i]);
				...
				chart4.Series[0].Points.DataBindXY(x, y4);
				chart5.Series[0].Points.DataBindXY(x, y5);
			}
